...
def run_cli(arguments: Sequence[str], cwd: Path | None = None) -> int:
    completed = subprocess.run([*resolve_cli(), *arguments], cwd=cwd or Path.cwd(), check=False)
    return completed.returncode


def approval_flags(args: argparse.Namespace) -> list[str]:
    flags: list[str] = []
